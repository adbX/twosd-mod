...
# The first warehouse may become not usable.
# If it fails then it is as if we ordered zero quantity.
alpha = 0.10
theta_d = Bernoulli(1.0 - alpha)
...
@constraint(model, con1[i=2:N], f[i]+sum(T[i,j] for j = 1:N if j != i) + e[i] - s[i] == 0)
# -theta * s[1]
@constraint(model, con11, f[1]+sum(T[1,j] for j = 1:N if j != 1) + e[1] - s[1] == 0)
# RHS: d_i
...
split_position = Position(con1[2], e[1])
...
# @show decision.(s, Ref(solution), CompromiseSolution)
# @show decision.(s, Ref(solution), AverageSolution)
# @show decision.(s, Ref(solution), IncumbentSolution)